...
### 7. historical-precedent-search.md — Dynamic Historical Precedent Search (NEW v3.1)

- **Purpose**: Guides the LLM in dynamically searching for historical precedents using `网络检索工具` when static cases are insufficient
- **Search Query Generation**: Automated query templates per event type, producing 1-4 optimized queries
- **Result Evaluation Rubric**: Match quality scoring (mechanism match, magnitude match, macro context match)
- **Extraction Protocol**: How to extract actionable data from search results (ticker moves, percentages, timelines, lessons)
- **Discount Rules**: Old cases (>10yr) need "world has changed" check; different macro regimes may not apply
- **Integration**: Triggered when mr_if_reason output's Dynamic Historical Search section is [STRONGLY RECOMMENDED] or [RECOMMENDED]

---

## Output Format Bifurcation (NEW v3.1.1)

The `mr_if_reason` tool now outputs **different formats** based on event type:

**Daily-Life Events** → **Chain Template format**:
- Pattern, Disciplines, Typical Steps, Sector hints, Ticker seeds (Bullish/Bearish)

**Financial Events** → **Transmission Channel format**:
- Mechanism, Time horizon, Winners/Losers, Priced-in risk, Key question, Expected magnitude

Section header also changes: "Chain Templates" vs "Transmission Channels".

### Financial Event Tool Priorities

When the input is a financial event, conditional tools have adjusted priorities:

| Tool | Priority | Why |
|------|----------|-----|
| 网络检索工具 | HIGH | Verify priced-in status, latest consensus shifts |
| rating_filter | HIGH | Analyst consensus alignment/divergence |
| top_gainers/top_losers | HIGH | Confirm market rotation direction |
| volume_breakout_scanner | HIGH | Institutional positioning signals |
| dcf计算工具 | MEDIUM | "Priced in?" quantification |
| 基于历史的股票收益预测器 | MEDIUM | Historical pattern confirmation |
| 贪婪先生数据获取工具 | MEDIUM | Sentiment regime context |
| 蒙特卡洛预测 | LOW | Cleaner data than daily events, less needed |
...
| Skills | 7 (+2 financial-transmission, historical-precedent-search v3.1) |
...
| Output format modes | 2 (Chain Template for daily events, Transmission Channel for financial events) |
| Dynamic search query templates | 11 event types covered |
| Worked examples in system prompt | 2 (daily-life event + financial event) |

---

*Document generated: 2026-02-09 | Version: 3.1.1 | Project: chaos2alpha (Mr.IF)*
